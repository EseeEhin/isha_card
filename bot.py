...
class TarotBot(commands.Bot):
    def __init__(self):
        ...
        self.updown_file = os.path.join(self.data_dir, 'updown_game.json')
        self.updown_data = self.load_data(self.updown_file)
        if not isinstance(self.updown_data, dict) or 'channels' not in self.updown_data:
...
# --- 楼上楼下游戏指令 ---
updown_group = app_commands.Group(name="楼上楼下", description="匿名楼上楼下游戏")

@updown_group.command(name="注册", description="将此频道注册为楼上楼下游戏频道")
@app_commands.checks.has_permissions(administrator=True)
async def register_updown_channel(interaction: discord.Interaction):
    channel_id_str = str(interaction.channel_id)
    
    if channel_id_str in bot.updown_data['channels']:
        await interaction.response.send_message("✅ 这个频道已经是楼上楼下游戏频道了。", ephemeral=True)
        return

    bot.updown_data['channels'][channel_id_str] = {
        "is_active": True,
        "message_history": []
    }
    bot.save_data(bot.updown_file, bot.updown_data)
    
    log_message = f"User '{interaction.user}' (ID: {interaction.user.id}) registered channel '{interaction.channel.name}' (ID: {interaction.channel_id}) for UpDown game."
    op_logger.info(log_message)

    await interaction.response.send_message(f"✅ 成功将 **{interaction.channel.name}** 注册为楼上楼下游戏频道！", ephemeral=True)

@register_updown_channel.error
async def register_updown_channel_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("❌ 你需要管理员权限才能执行此操作。", ephemeral=True)
    else:
        await interaction.response.send_message("❌ 发生未知错误。", ephemeral=True)
        logger.error(f"Error in register_updown_channel: {error}")
...
class UpDownInputModal(discord.ui.Modal, title="楼上楼下"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 将新消息添加到历史记录
        channel_data["message_history"].append(user_input)
        # 限制历史记录长度，防止无限增长
        if len(channel_data["message_history"]) > 50:
            channel_data["message_history"].pop(0)
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        await interaction.response.send_message('❌ 提交时发生错误，请重试。', ephemeral=True)
        logger.error(f"Error in UpDownInputModal: {error}")
